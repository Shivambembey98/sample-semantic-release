...
describe("createHistoryUser", () => {
   let req; let res;
   
   beforeEach(() => {
      req = {
         user: { id: 1 },
         body: { userId: 123, rentalDetails: "Details about the rental" },
         ip: '127.0.0.1',
         headers: {},
      };
      res = {
   ...
   test('should successfully create rental history', async () => {
      inputValidation.mockResolvedValue(null);
      knex.mockImplementationOnce(() => ({
         insert: jest.fn().mockReturnThis(),
         returning: jest.fn().mockResolvedValue([1]),
      }));
      await productController.createHistoryUser(req, res);
   ...
   test('should return INPUT_VALIDATION if userId is missing', async () => {
