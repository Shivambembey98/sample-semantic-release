...
describe('deleteSubCategory', () => {
   ...
   test('should return NOT_FOUND if subcategory does not exist', async () => {
      ...
      knex.mockImplementationOnce(() => ({
         ...
         first: jest.fn().mockResolvedValue(null), // No subcategory found
      }));
   ...
   test('should successfully delete a subcategory', async () => {
      productValidator.mockResolvedValue(null); // No validation error
    
      knex.mockImplementationOnce(() => ({
         where: jest.fn().mockReturnThis(),
         first: jest.fn().mockResolvedValue({ id: 1, subCategoryName: 'testSubCategory' }), // Subcategory exists
      }));
